...
/* initialP2PArray():  allocates and initials pointer to pointer array -
   allocates ONE more than count pointers AND sets last to NULL
   ************************************************************/
void** initialP2PArray (int count, size_t size){
	...
	if ( ! array){
		printf ("initialP2PArray() Error; memory allocation.\n");
		return NULL;
	}
	...
	for (int i = 0; i < count; i++){
		...
		*mover = (void*) malloc (size);
		if ( ! *mover){
			printf ("initiaP2PArray(): Error; memory allocation.\n");
			return NULL;
		}
		memset(*mover, 0, size);
		...
		*mover = NULL; // essential - important

	return array;

}

// TODO freeP2PArray() needs void*** THREE?
