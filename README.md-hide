...
Program uses curlGetXrdsGPS() function to query overpass server for GPS, found
in my overpass-c-queies repository: https://github.com/waelhammoudeh/overpass-c-queries
...
connection to it, overpass server is defined and assumed to be on the local machine.
An output directory for the program is created in main() in the user home directory.
...
checks server URL for correct format and also whether we can or cannot connect
to it. ... uses getopt_long() ... sets verbose, check all input files, output 
is set to output file if specified otherwise output is set to standard output,
we do not over write existing output file unless told to do so with force option.

Main structures are POINT, RECTANGLE, XROADS (cross roads) others like DIMENSION,
DL_LIST and DL_ELEM. The list is doubly liked list, note the list is the pointer
to data member is just a pointer, as you use this list structure stay aware of
the pointer as long as you need it, do not over write it or delete it when you
still need it. This pointer can be to any type, know what it points to all the 
time, do not mix pointers to different  types. Another general structure is the 
...
horses for this program. See README.structures file (incomplete) please.
